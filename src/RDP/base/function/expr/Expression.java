...
import RDP.base.function.expr.helper.SpecialBinOpHandler;
...
public interface Expression {
    ...
    static boolean ObjectIntoBoolean(Object e) {
        ...
        if (e instanceof Number n) return n.doubleValue() != 0.0;
    ...
    static ParseSuccess<Expression> parseExprLevel(int level, String input) throws ParseNomException {
        if (level >= LEVELS.length) return parseFactor0(input);
    ...
    private static SpecialBinOpHandler makeHandlerIs_IsNot(CompareOp value) {
        return (left, input) -> {
            ...
            if (t.status != TokenKind.NULLVALUE) throw new AfterIsOrIsNotErr(input);
